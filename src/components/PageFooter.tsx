 ...
 */
export default function PageFooter({ children, className = "" }: { children: ReactNode; className?: string }) {
  ...
  if (!slot) return null;
  return createPortal(<p className={`footer-note ${className}`.trim()}>{children}</p>, slot);
}
